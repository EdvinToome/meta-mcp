...
2. Resolve the site profile from `~/.meta-mcp/site-profiles.local.json` by slug, label, domain, or destination URL.
...
- Produce both `copy_context` and `copy_variants` before the structured build call.
...
- `copy_variants` must contain `parents`, `teachers`, and `general`, each with `headline` and `primary_text`.
