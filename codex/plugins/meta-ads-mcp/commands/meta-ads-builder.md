If `~/.meta-mcp/site-profiles.local.json` is missing, stop and ask the user to run `/meta-mcp-init` first.

Use the workflow in `skills/meta-ads-builder/SKILL.md`.

Start by:
1. Checking the `meta-ads-mcp` MCP server with `health_check` and `get_capabilities`.
2. Resolving the site profile from `~/.meta-mcp/site-profiles.local.json`.
