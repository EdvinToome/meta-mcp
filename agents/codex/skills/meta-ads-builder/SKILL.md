...
4. Propose the campaign structure before copy generation.
   ...
   - Do not call `ad_copy_writer` or `run_structured_ad_build` until the user approves the structure.
5. For each approved ad, delegate copy generation to subagent `ad_copy_writer` separately (one subagent call per ad):
   ...
   - if the gate fails for an ad, request a rewrite from `ad_copy_writer` for that ad before proceeding
